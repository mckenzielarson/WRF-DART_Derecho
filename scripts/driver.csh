...
set datefnl   =  2017043018 # target date   YYYYMMDDHH  # set this appropriately #%%%#
########################################################################
...
# MLL moving this within the while loop
# touch $RUN_DIR/cycle_started_${datea}

while ( 1 == 1 )
   touch $RUN_DIR/cycle_started_${datea} ## MLL moved this here
   ...
   set n = 1
   while ( $n <= $NUM_ENS) ## MLL added this for debugging && ${datea} != 2017042906
      ...
      while ( $n <= $NUM_ENS) ## MLL added this for debugging && ${datea} != 2017042906
         if (  -e    ${RUN_DIR}/ic_d${dchar}_${n}_ready) then
            ...
            if ( $loop > 60 ) then    # wait 5 minutes for the ic file to be ready, else run manually
               ...
               #MLL I am going to try this:
               $sub_command -v mem_num=${n},date=${datep},domain=${domains},paramf=${paramfile} ${SHELL_SCRIPTS_DIR}/prep_ic.c
   ...
   else if ( $SUPER_PLATFORM == 'derecho') then # MLL added this for debugging && ${datea} != 2017042906)
   ...
   set this_filter_runtime = $FILTER_TIME # MLL added this for ensemble members who are done (debugging) 
   ...
   #if (${datea} != 2017042706) then	## MLL added this for debugging
   ...
   while ( $n <= $NUM_ENS )
	 ...
	 if ( ! -e ${RUN_DIR}/done_member_${n} ) then ##MLL added this for restarts from segfaults
         	sed -f script.sed ${SHELL_SCRIPTS_DIR}/assim_advance.csh >! assim_advance_mem${n}.csh
   ...
   while ( $n <= $NUM_ENS )
      ...
      while ( $keep_trying == 'true' )
         ...
         sleep 10 # MLL added this because it wasn't waiting long enough
